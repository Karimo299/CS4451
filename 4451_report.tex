...
Brain cancer is one of the most severe and life-threatening conditions to be diagnosed with. According to the Global Cancer Observatory, approximately 320,000 people died from brain and central nervous system cancer in 2022 alone \cite{gco2025}. In Canada, it is estimated that 3300 people will be diagnosed with brain cancer, of which 78\% will die as a result of that cancer in 2025 \cite{cancercanada2025}. Early detection plays a major role in improving the patient's outcome, which can be the difference between a full recovery and death. Research indicates that earlier diagnosis and intervention significantly improve survival, with one study noting that a diagnosis within two weeks of symptom appearance and surgical intervention within three weeks is associated with prolonging the patient's survival \cite{kawauchi2025}.


Typically, brain tumors are detected by radiologists who examine MRI scans for abnormalities. This relies heavily on certified professionals to take the time to detect tumors. Studies also show that radiologist overburden is a major barrier to timely diagnosis, with nearly 29.7\% of delayed MRI and CT reports caused specifically by radiologists having too many scans to interpret \cite{wahid2025}. Additionally, tumors come in different shapes, sizes, contrasts, and textures, making it difficult to identify. While some of them can be easy to identify, others can be challenging to detect, especially early on. 


These challenges have contributed to the growing interest in using machine learning to support medical imaging analysis. A well-designed model can pick up subtle patterns that may not be immediately obvious to the human eye, helping improve early detection, increase consistency, and reduce the workload placed on radiologists. Despite substantial work in this area, most studies rely exclusively on image-only inputs, or deep learning pipelines. Our work is different as it directly compares first-order and second-order features alongside tabular features and tests how well they can predict tumors, while also checking whether adding the images improves performance. It offers a simpler approach than most existing studies.
